...
int main()
{
    ...
    // =======================   optimized icp   =======================
    // 补全相关代码
    OptimizedICPGN *opt=new OptimizedICPGN();
    opt->SetTargetCloud(cloud_target_ptr);
    opt->SetMaxIterations(30);
    opt->SetMaxCorrespondDistance(0.3f);
    opt->SetTransformationEpsilon(1e-6f);
    bool ok = opt->Match(cloud_source_ptr,T_predict,cloud_source_opti_transformed_ptr,T_final);
    std::cout << "\n==============optimized  ICP =================" << std::endl;
    std::cout << "T final: \n" << T_final << std::endl;
    // HasConverged() 返回是否收敛
    // GetFitnessScore() 返回匹配精度（越小越好）
    // std::cout << "Has converged: " << (opt->HasConverged() ? "yes" : "no") << std::endl;
    std::cout << "fitness score: " << opt->GetFitnessScore(0.3f) << std::endl;
    // =======================   optimized icp   =======================
